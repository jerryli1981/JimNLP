function MyScript(model_f, data_f, output_f)

load(model_f);
load(data_f);

batch_size = 1;
%b=ceil(length(valid_lbl)/batch_size);
b = 1;
valid_batch = reshape(valid',1,[]);
[train_msk, valid_msk, test_msk, p] = Masks(train, train_lbl, valid, valid_lbl, test, test_lbl, p);
valid_batch(((b-1)*batch_size*p(2))+1:min(b*batch_size*p(2),end));
valid_lbl((b-1)*batch_size+1:min(b*batch_size,end));
valid_msk((b-1)*batch_size+1:min(b*batch_size,end),:);
