package edu.pengli.nlp.platform.util.matrix;
...
public class Matrix implements Cloneable, java.io.Serializable {
	...
	public double sumCol(int k){
		double ret = 0.0;
		double[] vals = this.getCol(k);
		for(int i=0; i<vals.length; i++){
			ret += vals[i];
		}
		return ret;
	...
	/*
	 * square root of product of th row and column dimensions of a matrix
	 */
	public double normN() {
		Matrix tmp  = this.times(this.transpose());
		return Math.sqrt(tmp.sum());
	}
	
	public Matrix subMatrixAtUpperLeft(int i, int j){
	...
	public Matrix subMatrixAtLowerRight(int i, int j){
		Matrix M = new Matrix(m-i, n-j);
		for(int x=0; x< m-i; x++){
			for(int y=0; y< n-j; y++){
				M.set(x, y, A[x+i][y+j]);
			}
